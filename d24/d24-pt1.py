...
class Agent:
    ...
    def get_valid_positions_to_go(self, map):
        ...
        for (di, dj) in valid_dir:
            next_i = self.i + di
            next_j = self.j + dj
            if next_i > 0 and next_j > 0 and map[next_i][next_j] == EMPTY:
...
class Valley:
    def __init__(self, map_scan) -> None:
        ...
        self.initial_state = State(agent)
    ...
    def fastest_path_time(self) -> int:
        ...
        pq = PriorityQueue()
        visited = dict()
        pq.put(((0, get_manhattan_dist(self.start_pos, self.end_pos)), self.initial_state))
...
print(snow_valley.fastest_path_time())
...
def dynamic_programming_strategy(network: Network, start: str, time: int, valves_to_open: set):
    # initializing table
        ...
        for i in range(len(valves_names_list)):
            ...
            if table[i][t].pr != UNDEF:
                ...
                for next_v_n in valve.dist_table.keys():
                    ...
                    if not next_v_n in table[i][t].v_open and next_v_n in valves_to_open:
                        if t + d + 1 <= time:
...
for valve_n in net.valves.keys():
    if net.valves[valve_n].fr > 0:
        all_valves_to_open.add(valve_n)

max_duo_pr = - INF
for k in range(len(all_valves_to_open)//2 + 1):
    for subset in combinations(list(all_valves_to_open), k):
        valves_I_will_open = set()
        for valve_n in list(subset):
            valves_I_will_open.add(valve_n)
        valves_elephant_will_open = all_valves_to_open.difference(valves_I_will_open)
        
        my_pr = dynamic_programming_strategy(net, 'AA', 26, valves_I_will_open)
        elephant_pr = dynamic_programming_strategy(net, 'AA', 26, valves_elephant_will_open)

        if my_pr + elephant_pr > max_duo_pr:
            max_duo_pr = my_pr + elephant_pr
print(max_duo_pr)

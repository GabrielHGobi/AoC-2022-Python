...
costs_of_elevation_a_spots = []
for i in range(M):
    for j in range(N):
        if grid[i][j] == 'a':
            cost = BFS(grid, size, (i, j), (Ei, Ej))
            if cost > 0:
                costs_of_elevation_a_spots.append(cost)

costs_of_elevation_a_spots.sort()
print(costs_of_elevation_a_spots[0])